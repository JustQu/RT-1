...
/*   Updated: 2020/08/23 20:19:54 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mouse_move(t_rt *rt, t_all_rect *all_rect, t_colors *color)
{
	...
	if (is_press_button(rt, &all_rect->tab_main_button))
	{
		...
		render_tab_bar(rt, &color->tab_pressed_color,
			&all_rect->tab_main_button, WIDTH - WIDTH_MENU / 2 - WIDTH_MENU / 4, "MAIN");
		render_tab_bar(rt, &color->title_text_color,
			&all_rect->tab_render_button, WIDTH - WIDTH_MENU / 2 + WIDTH_MENU / 4, "RENDER");
		hlineRGBA(rt->sdl.render, WIDTH_OFFSET + MARGIN, WIDTH - WIDTH_MENU / 2,
								(FONT_TITLE_SIZE + MARGIN_Y) * 4 - MARGIN_Y,
		color->tab_pressed_color.r, color->tab_pressed_color.g,
								color->tab_pressed_color.b, 255);
		hlineRGBA(rt->sdl.render, WIDTH - WIDTH_MENU / 2, WIDTH - MARGIN,
				(FONT_TITLE_SIZE + MARGIN_Y) * 4 - MARGIN_Y, 217, 217, 217, 255);
	}
