...
/*   Updated: 2020/10/13 15:18:02 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_rect(t_all_rect *rect, t_window *win)
{
	...
	rect->rotate_button = init_rect_size(win->width - win->width / 4,
		rect->radius_button.y + rect->radius_button.h + MARGIN_Y * 2, win->width / 4, 30);

	rect->color_button = init_rect_size(win->width - win->width / 4,
		rect->rotate_button.y + rect->rotate_button.h + MARGIN_Y * 2, win->width / 4, 30);

	rect->diffuse_button = init_rect_size(win->width - win->width / 4,
	...
	rect->alpha_button = init_rect_size(win->width - win->width / 4,
		rect->reflection_button.y + rect->reflection_button.h + MARGIN_Y * 2, win->width / 4, 30);

	rect->samplers_button = init_rect_size(win->width - win->width / 4,
		rect->type_button.y + rect->type_button.h + MARGIN_Y * 2, win->width / 4, 30);

	rect->deep_button = init_rect_size(win->width - win->width / 4,
	...
	rect->type_choise_rect = init_rect_size(rect->type_button.x + MARGIN,
		rect->type_button.y, rect->type_button.w - MARGIN * 2, 30);
}
